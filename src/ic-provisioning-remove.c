		...
		OCDoHandle handle, OCClientResponse *resp)
{
	...
	if (OC_STACK_RESOURCE_DELETED != resp->result) {
		...
		if (container->cb_data->cb)
			container->cb_data->cb(container->cb_data->device_id, container->dest_device,
					IOTCON_ERROR_IOTIVITY, 0, container->cb_data->user_data);
			icl_provisioning_free_remove_delete_container(container);
		return OC_STACK_DELETE_TRANSACTION;
	}
...
static gboolean _provisioning_remove_device_delete_timeout(gpointer p)
{
	...
	if (container->cb_data->cb) {
		container->cb_data->cb(container->cb_data->device_id, NULL, IOTCON_ERROR_TIMEOUT,
				0, container->cb_data->user_data);
	}
		...
		iotcon_provisioning_remove_device_cb cb, void *user_data)
{
	...
	/* Get Linked devices */
	ret = PDMGetLinkedDevices(uuid, &container->linked_devices, &num_of_devices);
	if (OC_STACK_OK != ret) {
