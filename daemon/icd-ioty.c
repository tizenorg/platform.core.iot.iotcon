...
int icd_ioty_unregister_resource(OCResourceHandle handle)
{
	...
	if (OC_STACK_OK != ret) {
		...
		return ic_ioty_convert_error(ret);
	}
		...
		const char *bus_name)
{
	...
	oic_conn_type = ic_ioty_conn_type_to_oic_conn_type(conn_type);
		...
		GVariant *repr)
{
	...
	ret = ic_ioty_get_dev_addr(host, conn_type, &dev_addr);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("ic_ioty_get_dev_addr() Fail(%d)", ret);
	...
	if (OC_STACK_OK != result) {
		ERR("OCDoResource() Fail(%d)", result);
		icd_ioty_complete_error(type, invocation, ic_ioty_convert_error(result));
		return TRUE;
	}
		...
		const char *host_address, int conn_type)
{
	...
	ret = ic_ioty_get_dev_addr(host_address, conn_type, &encap_info->dev_addr);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("ic_ioty_get_dev_addr() Fail(%d)", ret);
		free(encap_info);
		return NULL;
	...
	encap_info->oic_conn_type = ic_ioty_conn_type_to_oic_conn_type(conn_type);
