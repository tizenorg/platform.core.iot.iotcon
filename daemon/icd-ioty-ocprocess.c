...
static int _worker_req_handler(void *context)
{
	...
	ret = ic_ioty_get_host_address(&ctx->dev_addr, &host_address, &conn_type);
	if (IOTCON_ERROR_NONE != ret) {
		...
		OCClientResponse *resp)
{
	...
	find_ctx->conn_type = ic_ioty_transport_flag_to_conn_type(resp->devAddr.adapter,
			resp->devAddr.flags);
		...
		OCClientResponse *resp)
{
	...
	case OC_STACK_OK:
		...
		ret = ic_ioty_convert_trigger_to_ioty(payload->trigger, &presence_ctx->trigger);
		if (IOTCON_ERROR_NONE != ret) {
...
static int _worker_encap_get_cb(void *context)
{
	...
	/* GET ENCAP INFO */
	ret = ic_ioty_get_host_address(&encap_get_ctx->dev_addr, &host_address, &conn_type);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("ic_ioty_get_host_address() Fail");
		return ret;
	}
...
static int _worker_encap_get(void *context)
{
	...
	ret = ic_ioty_get_host_address(&encap_ctx->dev_addr, &host_address, &conn_type);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("ic_ioty_get_host_address() Fail");
		return ret;
	}
