...
static int _worker_req_handler(void *context)
{
	...
	ret = ic_ioty_parse_oic_dev_address(&ctx->dev_addr, &host_address, &conn_type);
	if (IOTCON_ERROR_NONE != ret) {
		...
		OCEntityHandlerRequest *request, void *user_data)
{
	...
	if (OC_OBSERVE_FLAG & flag) {
		...
		req_ctx->observe_type = ic_ioty_parse_oic_action(request->obsInfo.action);
	} else {
		...
		OCClientResponse *resp)
{
	...
	find_ctx->conn_type = ic_ioty_parse_oic_transport(resp->devAddr.adapter,
			resp->devAddr.flags);
		...
		OCClientResponse *resp)
{
	...
	res = ic_ioty_parse_oic_response_result(resp->result);
		...
		OCClientResponse *resp)
{
	...
	case OC_STACK_OK:
		...
		presence_ctx->trigger = ic_ioty_parse_oic_trigger(payload->trigger);
		break;
...
static int _worker_encap_get_cb(void *context)
{
	...
	/* GET ENCAP INFO */
	ret = ic_ioty_parse_oic_dev_address(&encap_get_ctx->dev_addr, &host_address, &conn_type);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("ic_ioty_parse_oic_dev_address() Fail");
		return ret;
	}
...
static int _worker_encap_get(void *context)
{
	...
	ret = ic_ioty_parse_oic_dev_address(&encap_ctx->dev_addr, &host_address, &conn_type);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("ic_ioty_parse_oic_dev_address() Fail");
		return ret;
	}
