...
%else
mkdir -p %{buildroot}/opt/usr/data/%{name}
cp %{name}-network-get %{buildroot}/opt/usr/data/%{name}/%{name}-network-get
cp %{name}-internet %{buildroot}/opt/usr/data/%{name}/%{name}-internet
%endif

%post
%if 0%{?tizen_version_major} >= 3
chgrp priv_internet /opt/usr/data/%{name}/.%{name}-internet
chmod g+r /opt/usr/data/%{name}/.%{name}-internet
chmod o= /opt/usr/data/%{name}/.%{name}-internet
chsmack -a "*" /opt/usr/data/%{name}/.%{name}-internet

chgrp priv_network_get /opt/usr/data/%{name}/.%{name}-network-get
chmod g+r /opt/usr/data/%{name}/.%{name}-network-get
chmod o= /opt/usr/data/%{name}/.%{name}-network-get
chsmack -a "*" /opt/usr/data/%{name}/.%{name}-network-get
%endif
/sbin/ldconfig
...
/opt/usr/data/%{name}/%{name}-network-get
/opt/usr/data/%{name}/%{name}-internet
%endif
