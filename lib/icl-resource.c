		...
		iotcon_resource_h *resource_handle)
{
	...
	iface_array = icl_dbus_resource_ifaces_to_array(ifaces);
	if (NULL == iface_array) {
		ERR("icl_dbus_resource_ifaces_to_array() Fail");
		free(type_array);
		free(resource);
		return IOTCON_ERROR_OUT_OF_MEMORY;
	}
	...
	resource->ifaces = icl_resource_ifaces_ref(ifaces);
...
API int iotcon_resource_bind_interface(iotcon_resource_h resource, const char *iface)
{
	...
	ret = iotcon_resource_ifaces_clone(resource->ifaces, &resource_ifaces);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("iotcon_resource_ifaces_clone() Fail(%d)", ret);
		return ret;
	}

	ret = iotcon_resource_ifaces_add(resource_ifaces, iface);
	if (IOTCON_ERROR_NONE != ret) {
