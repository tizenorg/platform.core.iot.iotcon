		...
		icl_provisioning_remove_cb_container_s *container)
{
	...
	device = icl_provisioning_device_get_device(container->device);

	ret = PDMDeleteDevice(&device->doxm->deviceID);
	if (OC_STACK_OK != ret)
...
static void icl_provisioning_free_remove_delete_container(gpointer p)
{
	...
	icl_provisioning_free_remove_cb_container(container->cb_data);
...
static gboolean _provisioning_remove_idle_cb(gpointer p)
{
	...
	if (container->cb_data->cb) {
		container->cb_data->cb(container->cb_data->device, container->dest_device,
				IOTCON_ERROR_NONE, 0, container->cb_data->user_data);
	}

	icl_provisioning_free_remove_delete_container(container);
		...
		OCDoHandle handle, OCClientResponse *resp)
{
	...
	device = icl_provisioning_device_get_device(container->cb_data->device);

	ret = PDMUnlinkDevices(&device->doxm->deviceID, &device_id);
	if (OC_STACK_OK != ret) {
		...
		icl_provisioning_remove_delete_container_s *container)
{
	...
	device_id = icl_provisioning_parse_uuid(&device->doxm->deviceID);

	snprintf(uri, sizeof(uri), "%s%s%s?%s=%s", IC_IOTY_COAPS, host_address, cred_uri,
			subject_uuid, device_id);

	free(device_id);
	free(host_address);

	cbdata.cb = _provisioning_remove_device_delete_cb;
	cbdata.cd = icl_provisioning_free_remove_delete_container;
		...
		iotcon_error_e result, void *user_data)
{
	...
	container = calloc(1, sizeof(icl_provisioning_device_get_device));
	if (NULL == container) {
...
API int iotcon_provisioning_remove_device(iotcon_provisioning_device_h device,
		iotcon_provisioning_remove_device_cb cb, void *user_data)
{
	...
	OCProvisionDev_t *target_device;
	icl_provisioning_remove_cb_container_s *container;
	...
	target_device = icl_provisioning_device_get_device(container->device);

	/* Get Linked devices */
	ret = PDMGetLinkedDevices(&target_device->doxm->deviceID, &container->linked_devices,
			&num_of_devices);
	if (OC_STACK_OK != ret) {
		ERR("PDMGetLinkedDevices() Fail(%d)", ret);
		icl_provisioning_free_remove_cb_container(container);
		return ret;
	}
	...
	for (cur = container->linked_devices; cur; cur = cur->next) {
		ret = PDMSetLinkStale(&cur->dev, &target_device->doxm->deviceID);
		if (OC_STACK_OK != ret) {
