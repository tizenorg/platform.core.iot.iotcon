		...
		OCClientResponse *resp)
{
	...
	ret = icl_ioty_parse_oc_discovery_payload(&(resp->devAddr), (OCDiscoveryPayload*)resp->payload,
		...
		OCDoHandle handle, OCClientResponse* resp)
{
	...
	ret = icl_ioty_parse_oc_device_payload((OCDevicePayload *)resp->payload,
			&info);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("icl_ioty_parse_oc_device_payload() Fail(%d)", ret);
		if (cb_data->cb)
		...
		OCDoHandle handle, OCClientResponse* resp)
{
	...
	ret = icl_ioty_parse_oc_platform_payload((OCPlatformPayload *)resp->payload,
			&info);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("icl_ioty_parse_oc_platform_payload() Fail(%d)", ret);
		if (cb_data->cb)
		...
		OCClientResponse *resp)
{
	...
	ret = icl_ioty_parse_oc_presence_payload(&resp->devAddr, (OCPresencePayload*)resp->payload,
			resp->result, &presence_response);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("icl_ioty_parse_oc_presence_payload() Fail(%d)", ret);
		return OC_STACK_KEEP_TRANSACTION;
	}
		...
		OCDoHandle handle, OCClientResponse* resp)
{
	...
	/* representation */
	ret = icl_ioty_parse_oc_rep_payload((OCRepPayload*)resp->payload, true, &repr);
	if (IOTCON_ERROR_NONE != ret) {
	...
	/* response result */
	response_result = ic_ioty_parse_oc_response_result(resp->result);

	/* header options */
	ret = icl_ioty_parse_oc_header_option(resp->rcvdVendorSpecificHeaderOptions,
			resp->numRcvdVendorSpecificHeaderOptions, &options);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("icl_ioty_parse_oc_header_option() Fail(%d)", ret);
		return OC_STACK_KEEP_TRANSACTION;
	}
...
static gboolean _icl_ioty_ocprocess_response_idle_cb(gpointer p)
{
		...
		OCDoHandle handle, OCClientResponse* resp)
{
	...
	g_idle_add(_icl_ioty_ocprocess_response_idle_cb, response_cb_data);
		...
		OCEntityHandlerRequest *request, void *user_data)
{
	...
	struct icl_resource_request req = {0};

	RETV_IF(NULL == resource, OC_EH_ERROR);

	/* request type */
	if (OC_REQUEST_FLAG & flag)
		req_type = ic_ioty_parse_oc_method(request->method);
	else
		req_type = IOTCON_REQUEST_UNKNOWN;

	/* observe type */
	if (OC_OBSERVE_FLAG & flag) {
		observe_id = request->obsInfo.obsId | IC_OBSERVE_ID_MASK_FOR_LIB;
		obs_type = ic_ioty_parse_oc_action(request->obsInfo.action);
	} else {
		obs_type = IOTCON_OBSERVE_NO_TYPE;
	}

	/* header options */
	ret = icl_ioty_parse_oc_header_option(request->rcvdVendorSpecificHeaderOptions,
	...
	/* representation */
	if (request->payload)
		icl_ioty_parse_oc_rep_payload((OCRepPayload*)request->payload, true, &repr);
	else
		repr = NULL;
	...
	ic_ioty_parse_oc_dev_address(&request->devAddr, &host_address, &conn_type);
	...
	resource->cb(resource, &req, resource->user_data);
		...
		iotcon_lite_resource_h resource, iotcon_representation_h *representation)
{
	...
	ret = iotcon_representation_set_resource_interfaces(repr, IOTCON_INTERFACE_DEFAULT);
	if (IOTCON_ERROR_NONE != ret) {
		...
		OCEntityHandlerRequest *request, void *user_data)
{
	...
	iotcon_representation_h repr;
	struct icl_resource_response res = {0};
