...
static GMutex icl_ioty_polling_mutex;
...
API int iotcon_polling_set_interval(int interval)
{
	...
	icl_ioty_polling_interval = interval * 1000ll;

	g_mutex_lock(&icl_ioty_polling_mutex);
	g_cond_signal(&icl_ioty_polling_cond);
	g_mutex_unlock(&icl_ioty_polling_mutex);
...
gpointer icl_ioty_ocprocess_thread(gpointer data)
{
	...
	while (icl_ioty_alive) {
		...
		g_cond_wait_until(&icl_ioty_polling_cond, &icl_ioty_polling_mutex,
