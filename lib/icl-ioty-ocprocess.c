		...
		OCEntityHandlerRequest *request, void *user_data)
{
	...
	if (OC_REQUEST_FLAG & flag)
		req_type = ic_ioty_utils_parse_oic_method(request->method);
	else
	...
	if (OC_OBSERVE_FLAG & flag) {
		...
		obs_type = ic_ioty_utils_parse_oic_action(request->obsInfo.action);
	} else {
		obs_type = IOTCON_OBSERVE_NO_TYPE;
	}

	/* header options */
	options = _icl_ioty_ocprocess_parse_header_options(
	...
	/* representation */
	if (request->payload)
		icl_ioty_parse_representation_payload((OCRepPayload*)request->payload, true, &repr);
	else
		repr = NULL;
	...
	ic_ioty_utils_parse_oic_dev_address(&request->devAddr, &host_address, &conn_type);
