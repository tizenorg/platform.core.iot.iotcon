...
#include "icl-remote-resource-monitoring.h"
#include "icl-remote-resource-caching.h"
		...
		OCDoHandle *observe_handle)
{
	...
	/* host address, port */
	ret = ic_ioty_utils_convert_dev_address(resource->host_address,
		...
		void *user_data)
{
	...
	/* method (request type) */
	method = ic_ioty_utils_convert_request_type(req_type);
	...
	/* representation */
	if (repr) {
		ret = icl_ioty_create_repr_payload(repr, &payload);
		if (IOTCON_ERROR_NONE != ret) {
			ERR("icl_ioty_create_repr_payload() Fail(%d)", ret);
			free(uri);
			return ret;
		}
	}
...
static gboolean _icl_ioty_monitoring_timeout_cb(gpointer p)
{
	int ret;
		...
		iotcon_error_e err, iotcon_presence_response_h response, void *user_data)
{
	...
	cb_container->timeout = g_timeout_add_seconds(IC_REMOTE_RESOURCE_DEFAULT_TIME_INTERVAL,
			_icl_ioty_monitoring_timeout_cb, cb_container);
}
		...
		iotcon_remote_resource_state_changed_cb cb, void *user_data)
{
	...
	icl_remote_resource_monitoring_table_insert(resource, cb_container);
...
int icl_ioty_remote_resource_stop_monitoring(iotcon_remote_resource_h resource)
{
	...
	icl_remote_resource_monitoring_table_remove(resource);
...
static gboolean _icl_ioty_caching_timeout_cb(gpointer p)
{
	int ret;
		...
		iotcon_error_e err, int sequence_number, iotcon_response_h response, void *user_data)
{
	...
	if (cb_container->timeout)
		g_source_remove(cb_container->timeout);
	cb_container->timeout = g_timeout_add_seconds(IC_REMOTE_RESOURCE_DEFAULT_TIME_INTERVAL,
			_icl_ioty_caching_timeout_cb, cb_container);
}
		...
		iotcon_remote_resource_cached_representation_changed_cb cb, void *user_data)
{
	...
	/* get */
	ret = icl_ioty_remote_resource_get(resource, NULL, _icl_ioty_caching_get_cb,
			cb_container);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("icl_ioty_remote_resource_get() Fail(%d)", ret);
		icl_destroy_caching_container(cb_container);
	...
	icl_remote_resource_caching_table_insert(resource, cb_container);
...
int icl_ioty_remote_resource_stop_caching(iotcon_remote_resource_h resource)
{
	...
	icl_remote_resource_caching_table_remove(resource);
...
static int _icl_ioty_resource_bind_interface(OCResourceHandle handle, iotcon_interface_e iface)
{
	int ret;
	OCStackResult result;
		...
		iotcon_resource_h *resource_handle)
{
	...
	icl_resource_types_ref(resource->types);
	...
	ret = OCCreateResource(&handle, res_types->type_list->data, res_iface, uri_path,
			icl_ioty_ocprocess_request_cb, resource, properties);
	icl_ioty_csdk_unlock();
	if (OC_STACK_OK != ret) {
	...
	resource->handle_lib = IC_POINTER_TO_INT64(handle);
	...
	if (IOTCON_INTERFACE_DEFAULT & ifaces)
		_icl_ioty_resource_bind_interface(handle, IOTCON_INTERFACE_DEFAULT);
	if (IOTCON_INTERFACE_LINK & ifaces)
		_icl_ioty_resource_bind_interface(handle, IOTCON_INTERFACE_LINK);
	if (IOTCON_INTERFACE_BATCH & ifaces)
		_icl_ioty_resource_bind_interface(handle, IOTCON_INTERFACE_BATCH);
	if (IOTCON_INTERFACE_GROUP & ifaces)
		_icl_ioty_resource_bind_interface(handle, IOTCON_INTERFACE_GROUP);
	if (IOTCON_INTERFACE_READONLY & ifaces)
		_icl_ioty_resource_bind_interface(handle, IOTCON_INTERFACE_READONLY);
		...
		const char *resource_type)
{
	...
	handle = IC_INT64_TO_POINTER(resource->handle_lib);
		...
		iotcon_observers_h observers, iotcon_qos_e qos)
{
	...
	for (c = observers_ori->observers_list; c; c = c->next) {
		...
		if (IC_OBSERVE_ID_MASK_FOR_LIB & observe_id) {
			observe_id ^= IC_OBSERVE_ID_MASK_FOR_LIB;
	...
	ret = icl_ioty_create_repr_payload(repr, (OCPayload **)&payload);
	if (IOTCON_ERROR_NONE != ret) {
		ERR("icl_ioty_create_repr_payload() Fail(%d)", ret);
		return ret;
	}

	oc_qos = ic_ioty_utils_convert_qos(qos);
...
int icl_ioty_resource_destroy(iotcon_resource_h resource)
{
	...
	if (0 == resource->handle_lib) { /* iotcon dbus disconnected */
...
int icl_ioty_response_send(iotcon_response_h response_handle)
{
	...
	response.ehResult = ic_ioty_utils_convert_response_result(response_handle->result);
	if (response_handle->header_options) {
		_icl_ioty_get_header_options(response_handle->header_options->hash,
				response.sendVendorSpecificHeaderOptions,
				sizeof(response.sendVendorSpecificHeaderOptions)
				/ sizeof(response.sendVendorSpecificHeaderOptions[0]));
	}
	if (response_handle->repr)
		icl_ioty_create_repr_payload(response_handle->repr, &(response.payload));
