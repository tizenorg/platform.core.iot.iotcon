...
static int _icl_ioty_resource_bind_interface(OCResourceHandle handle, iotcon_interface_e iface)
{
	...
	resource_interface = ic_ioty_convert_interface(iface);
		...
		iotcon_resource_h child)
{
	...
	handle_parent = IC_INT64_TO_POINTER(parent->handle_lib);
	handle_child = IC_INT64_TO_POINTER(child->handle_lib);
