...
int icl_ioty_remote_resource_observe_deregister(iotcon_remote_resource_h resource)
{
	...
	icl_ioty_csdk_lock();
	ret = OCCancel(IC_INT64_TO_POINTER(resource->observe_handle), OC_HIGH_QOS,
			oic_options_ptr, options_size);
	icl_ioty_csdk_unlock();
		...
		iotcon_query_h query, iotcon_remote_resource_response_cb cb, void *user_data)
{
	...
	ret = _icl_ioty_remote_resource_crud(IOTCON_REQUEST_GET, resource, query, NULL,
			cb, user_data);
	if (IOTCON_ERROR_NONE != ret) {
