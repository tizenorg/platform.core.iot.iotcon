...
static int _icl_ioty_parse_oc_rep_payload_value(OCRepPayloadValue *val,
		iotcon_state_h *state);
static int _icl_ioty_fill_oc_rep_payload_value(OCRepPayload *payload,
		iotcon_state_h state);
...
int icl_ioty_parse_oc_discovery_payload(OCDevAddr *dev_addr,
		...
		int *resource_count)
{
	...
	for (i = 0; res_payload; i++, res_payload = res_payload->next) {
		...
		/* Resource Properties */
		properties = ic_ioty_parse_oc_properties(res_payload->bitmap);
		...
		/* connectivity type */
		conn_type = ic_ioty_parse_oc_transport(dev_addr->adapter, dev_addr->flags);
...
int icl_ioty_parse_oc_device_payload(OCDevicePayload *payload,
...
int icl_ioty_parse_oc_platform_payload(OCPlatformPayload *payload,
...
int icl_ioty_parse_oc_presence_payload(OCDevAddr *dev_addr,
		...
		iotcon_presence_response_h *presence_response)
{
	...
	ret = ic_ioty_parse_oc_dev_address(dev_addr, &(resp->host_address),
	...
	case OC_STACK_OK:
		...
		resp->trigger = ic_ioty_parse_oc_trigger(payload->trigger);
		break;
...
static int _icl_ioty_parse_oc_rep_payload_value_array_attr(
		OCRepPayloadValueArray *arr, int len, int index, iotcon_list_h *list)
{
	...
	case OCREP_PROP_OBJECT:
		...
		for (i = 0; i < len; i++) {
			iotcon_state_h state;
			ret = _icl_ioty_parse_oc_rep_payload_value(arr->objArray[index + i]->values, &state);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_parse_oc_rep_payload_value");
				iotcon_list_destroy(l);
				return ret;
			}
...
static int _icl_ioty_parse_oc_rep_payload_value_array(OCRepPayloadValueArray *arr,
		int depth, int len, int index, iotcon_list_h *list)
{
	...
	if ((MAX_REP_ARRAY_DEPTH - 1) == depth || 0 == arr->dimensions[depth + 1]) {
		ret = _icl_ioty_parse_oc_rep_payload_value_array_attr(arr, len, index, &l);
		if (IOTCON_ERROR_NONE != ret) {
			ERR("_icl_ioty_parse_oc_rep_payload_value_array_attr() Fail(%d)", ret);
			return ret;
		}
		*list = l;
		return IOTCON_ERROR_NONE;
	...
	while ((index += next_len) < i) {
		...
		ret = _icl_ioty_parse_oc_rep_payload_value_array(arr, depth + 1, next_len, index, &l_child);
		if (IOTCON_ERROR_NONE != ret) {
			ERR("_icl_ioty_parse_oc_rep_payload_value_array() Fail(%d)", ret);
			iotcon_list_destroy(l);
			return ret;
		}
		...
		iotcon_state_h *state)
{
	...
	while (val) {
		...
		case OCREP_PROP_ARRAY:
			...
			ret = _icl_ioty_parse_oc_rep_payload_value_array(&val->arr, 0, total_len, 0, &list);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_parse_oc_rep_payload_value_array() Fail(%d)", ret);
				iotcon_state_destroy(s);
				return ret;
			...
			break;
		case OCREP_PROP_OBJECT:
			ret = _icl_ioty_parse_oc_rep_payload_value(val->obj->values, &s_obj);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_parse_oc_rep_payload_value() Fail(%d)", ret);
				iotcon_state_destroy(s);
				return ret;
			}
...
int icl_ioty_parse_oc_rep_payload(OCRepPayload *payload, bool is_parent,
		iotcon_representation_h *representation)
{
	...
	if (node) {
		...
		ifaces = IOTCON_INTERFACE_DEFAULT; /* TODO: verify spec */
	}
	...
	/* state */
	if (payload->values) {
		ret = _icl_ioty_parse_oc_rep_payload_value(payload->values, &state);
		if (IOTCON_ERROR_NONE != ret) {
			ERR("iotcon_representation_set_resource_types() Fail(%d)", ret);
			iotcon_representation_destroy(repr);
			return ret;
	...
	while (is_parent && child_node) {
		...
		ret = icl_ioty_parse_oc_rep_payload(child_node, false, &repr_child);
		if (IOTCON_ERROR_NONE != ret) {
			ERR("icl_ioty_parse_oc_rep_payload() Fail(%d)", ret);
			iotcon_representation_destroy(repr);
			return ret;
		}
...
static int _icl_ioty_fill_oc_rep_payload_value_array(OCRepPayload *payload,
		const char *key, struct icl_state_list_s *list)
{
	...
	case OCREP_PROP_STRING:
		...
		for (node = list->list, i = 0; node; node = node->next, i++)
			str_arr[i] = strdup(((icl_basic_s*)node->data)->val.s); /* TODO: strdup? */
	...
	case OCREP_PROP_OBJECT:
		...
		for (node = list->list, i = 0; node; node = node->next, i++) {
			...
			ret = _icl_ioty_fill_oc_rep_payload_value(state_arr[i],
					((icl_val_state_s*)node->data)->state);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_fill_oc_rep_payload_value() Fail(%d)", ret);
				return ret;
			}
		}
		...
		iotcon_state_h state)
{
	...
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		...
		case IOTCON_TYPE_BYTE_STR:
			...
			break;
		case IOTCON_TYPE_LIST:
			ret = _icl_ioty_state_list_to_value_list(((icl_val_list_s*)state_value)->list, &value_list, 0);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_state_list_to_value_list() Fail(%d)", ret);
				return ret;
			}
			ret = _icl_ioty_fill_oc_rep_payload_value_array(payload, key, &value_list);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_fill_oc_rep_payload_value_array() Fail(%d)", ret);
		...
		case IOTCON_TYPE_STATE:
			...
			ret = _icl_ioty_fill_oc_rep_payload_value(repr_payload, ((icl_val_state_s*)state_value)->state);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_fill_oc_rep_payload_value() Fail(%d)", ret);
...
int icl_ioty_convert_representation(iotcon_representation_h repr, OCPayload **payload)
{
	FN_CALL;
	int ret, i, ifaces = 0;
	GList *c;
	...
	if (ifaces) {
		for (i = 1; i <= IC_INTERFACE_MAX; i = i << 1) {
			...
			OCRepPayloadAddInterface(repr_payload, iface_str);
		}
	} else {
		OCRepPayloadAddInterface(repr_payload, IC_INTERFACE_DEFAULT);
	}
	...
	/* state */
	if (repr->state) {
		ret = _icl_ioty_fill_oc_rep_payload_value(repr_payload, repr->state);
		if (IOTCON_ERROR_NONE != ret) {
			ERR("_icl_ioty_fill_oc_rep_payload_value() Fail(%d)", ret);
			OCRepPayloadDestroy(repr_payload);
			return ret;
...
int icl_ioty_parse_oc_header_option(OCHeaderOption *option, int option_size,
