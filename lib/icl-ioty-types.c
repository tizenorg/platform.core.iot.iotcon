		...
		OCRepPayloadValueArray *arr, int len, int index, iotcon_list_h *list)
{
	...
	case OCREP_PROP_OBJECT:
		...
		for (i = 0; i < len; i++) {
			iotcon_state_h state;
			ret = _icl_ioty_parse_oic_rep_payload_value(arr->objArray[index + i]->values, &state);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_parse_oic_rep_payload_value");
				iotcon_list_destroy(l);
				return ret;
			}
...
static int _icl_ioty_parse_oic_rep_payload_value_array(OCRepPayloadValueArray *arr,
		int depth, int len, int index, iotcon_list_h *list)
{
	...
	while ((index += next_len) < i) {
		...
		ret = _icl_ioty_parse_oic_rep_payload_value_array(arr, depth + 1, next_len, index, &l_child);
		if (IOTCON_ERROR_NONE != ret) {
		...
		iotcon_state_h *state)
{
	...
	while (val) {
		...
		case OCREP_PROP_ARRAY:
			...
			ret = _icl_ioty_parse_oic_rep_payload_value_array(&val->arr, 0, total_len, 0, &list);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_parse_oic_rep_payload_value_array() Fail(%d)", ret);
				iotcon_state_destroy(s);
				return ret;
		...
		iotcon_state_h state)
{
	...
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		...
		case IOTCON_TYPE_STATE:
			...
			ret = _icl_ioty_fill_oic_rep_payload_value(repr_payload, ((icl_val_state_s*)state_value)->state);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_fill_oic_rep_payload_value() Fail(%d)", ret);
