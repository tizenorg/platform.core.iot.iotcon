...
static int _icl_ioty_fill_repr_payload_value(OCRepPayload *payload,
		iotcon_state_h state);
		...
		OCStackResult result, iotcon_presence_response_h *presence_response)
{
	...
	case OC_STACK_OK:
		...
		ret = ic_ioty_utils_parse_oic_trigger(payload->trigger, &resp->trigger);
		if (IOTCON_ERROR_NONE != ret) {
			ERR("ic_ioty_utils_parse_oic_trigger() Fail(%d)", ret);
			icl_destroy_presence_response(resp);
			return ret;
		}
		break;
...
static int _icl_ioty_fill_repr_payload_value_array(OCRepPayload *payload,
		const char *key, struct icl_state_list_s *list)
{
	...
	case OCREP_PROP_OBJECT:
		...
		for (node = list->list, i = 0; node; node = node->next, i++) {
			...
			ret = _icl_ioty_fill_repr_payload_value(state_arr[i],
					((icl_val_state_s*)node->data)->state);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_fill_repr_payload_value() Fail(%d)", ret);
				return ret;
			}
		}
		...
		iotcon_state_h state)
{
	...
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		...
		case IOTCON_TYPE_LIST:
			...
			ret = _icl_ioty_fill_repr_payload_value_array(payload, key, &value_list);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_fill_repr_payload_value_array() Fail(%d)", ret);
		...
		case IOTCON_TYPE_STATE:
			...
			ret = _icl_ioty_fill_repr_payload_value(repr_payload, ((icl_val_state_s*)state_value)->state);
			if (IOTCON_ERROR_NONE != ret) {
				ERR("_icl_ioty_fill_repr_payload_value() Fail(%d)", ret);
...
int icl_ioty_create_repr_payload(iotcon_representation_h repr, OCPayload **payload)
{
	FN_CALL;
	int ret, i, ifaces = 0;
	GList *c;
	...
	/* state */
	if (repr->state) {
		ret = _icl_ioty_fill_repr_payload_value(repr_payload, repr->state);
		if (IOTCON_ERROR_NONE != ret) {
			ERR("_icl_ioty_fill_repr_payload_value() Fail(%d)", ret);
			OCRepPayloadDestroy(repr_payload);
			return ret;
	...
	for (c = repr->children; c; c = c->next) {
		...
		ret = icl_ioty_create_repr_payload(c->data, &child);
		if (IOTCON_ERROR_NONE != ret) {
