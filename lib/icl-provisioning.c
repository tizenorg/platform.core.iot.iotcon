...
static void _provisioning_input_pin_cb(char *pin, size_t len)
{
	...
	temp = icl_randompins_cb_container.cb(len, icl_randompins_cb_container.user_data);
	if (len != strlen(temp) + 1) {
		ERR("Invalid pin length(%d)", strlen(temp) + 1);
		return;
	}
		...
		void *user_data)
{
	...
	result = OCProvisionCredentials(NULL,
			...
			key_size,
			icl_provisioning_device_get_device(device1),
			icl_provisioning_device_get_device(device2),
